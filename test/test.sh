...
agent --resume=AGT" "$(cat session.txt)"
...
check "dead entry pruned, 6 live kept" \
"claude --resume AAA  # Renamed
codex resume CDX
opencode --session ses_OC  # OC
pi --session PI1
...
check "resume show -> newest command"       "agent --resume=AGT"              "$(HOME="$HM" bash "$RESUME" show 2>/dev/null)"
...
for t in claude codex opencode pi cursor-agent agent; do
  printf '#!/bin/sh\necho "RAN %s $*"\n' "$t" > "$BIN/$t"; chmod +x "$BIN/$t"
done
check "resume runs newest (agent, fused)"  "RAN agent --resume=AGT"          "$(cd "$WORK" && HOME="$HM" PATH="$BIN:$PATH" bash "$RESUME" 2>/dev/null)"
check "resume 4 runs pi"                    "RAN pi --session PI1"            "$(cd "$WORK" && HOME="$HM" PATH="$BIN:$PATH" bash "$RESUME" 4 2>/dev/null)"
check "resume 5 runs cursor-agent (fused)"  "RAN cursor-agent --resume=CUR1"  "$(cd "$WORK" && HOME="$HM" PATH="$BIN:$PATH" bash "$RESUME" 5 2>/dev/null)"
